#!/usr/bin/env python3
"""
Simple test script to verify the critical fixes for the Luma cog.
"""
...
def test_event_model_validation():
    ...
    try:
        ...
        host = Host(**host_data)
        print(
            f"SUCCESS: Host model validation passed: {host.name} (tiktok: {host.tiktok_handle})"
        )
        ...
        print(f"SUCCESS: Event model validation passed: {event.name}")
...
def test_database_upsert_logic():
    ...
    try:
        ...
        async def test_first_upsert():
            ...
            print("SUCCESS: First upsert successful")
            ...
            print("SUCCESS: Duplicate upsert successful (no UNIQUE constraint error)")
        ...
        print(f"FAILED: Database upsert test failed: {e}")
        import traceback
...
def test_event_object_access():
    ...
    try:
        ...
        print(f"SUCCESS: Event object access successful:")
        ...
        print(f"FAILED: Event object access test failed: {e}")
        import traceback
...
async def test_integration():
    ...
    try:
        ...
        print(f"SUCCESS: Successfully created {len(events)} Event objects")

        # Test database operations
        db = EventDatabase(db_path=":memory:")
        result = await db.upsert_events(events_data, "cal_123")
        print(f"SUCCESS: Database upsert successful: {result}")
        ...
        host = Host(**host_data)
        print(
            f"SUCCESS: Host creation successful: {host.name} (tiktok: {host.tiktok_handle})"
        )
